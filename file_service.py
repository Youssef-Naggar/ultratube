...
class FileService:
    ...
    def merge_subtitles(
        ...
        if not output_file:
            ...
            # Use the original file's extension for the new merged file
    ...
    def process_download(self, file_path: str, options: ProcessOptions) -> str:
        """
        ...
        try:
            ...
            output_file = os.path.join(
                directory, f"{base_name}_processed.{options.output_format}"
            )

            # Basic FFmpeg command with format conversion
            command = ["ffmpeg", "-i", file_path, "-c:v", "copy", "-c:a", "copy"]
    ...
    def get_ffmpeg_path(self) -> str:
        ...
        Works in development and when bundled via PyInstaller.
        """
        ...
        if meipass:
            for filename in ["ffmpeg.exe", "ffmpeg"]:
                ...
                if os.path.exists(path):
                    return path
                # Check bin folder
                path = os.path.join(meipass, "bin", filename)
                if os.path.exists(path):
                    return path
        ...
        try:
            ...
            # Log error internally or fallback
            pass
    ...
    def _run_ffmpeg_command(self, command: List[str]) -> str:
        ...
        if command and command[0] == "ffmpeg":
            command[0] = self.get_ffmpeg_path()
